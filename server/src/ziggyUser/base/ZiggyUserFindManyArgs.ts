/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ZiggyUserWhereInput } from "./ZiggyUserWhereInput";
import { Type } from "class-transformer";
import { ZiggyUserOrderByInput } from "./ZiggyUserOrderByInput";

@ArgsType()
class ZiggyUserFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ZiggyUserWhereInput,
  })
  @Field(() => ZiggyUserWhereInput, { nullable: true })
  @Type(() => ZiggyUserWhereInput)
  where?: ZiggyUserWhereInput;

  @ApiProperty({
    required: false,
    type: [ZiggyUserOrderByInput],
  })
  @Field(() => [ZiggyUserOrderByInput], { nullable: true })
  @Type(() => ZiggyUserOrderByInput)
  orderBy?: Array<ZiggyUserOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ZiggyUserFindManyArgs };
